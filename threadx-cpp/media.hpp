...
template <MediaSectorSize N = defaultSectorSize> class Media : ThreadX::Native::FX_MEDIA, MediaBase
{
  public:
    ...
    // Once initialized by this constructor, the application should call fx_system_date_set and fx_system_time_set to
    // start with an accurate system date and time.
    explicit Media(const DriverCallback driverCallback,
                   std::byte *driverInfoPtr = nullptr,
                   const NotifyCallback openNotifyCallback = {},
    ...
    auto format(const std::string_view volumeName,
                const ThreadX::Ulong storageSize,
                const ThreadX::Uint sectorPerCluster = 1,
  ...
  private:
    ...
    static_assert(m_faultTolerantMinimunCacheSize % ThreadX::wordSize == 0,
    ...
    static constexpr auto cacheSize = []() {
        return (std::to_underlying(N) > std::to_underlying(MediaSectorSize::oneKiloByte))
                   ? std::to_underlying(MediaSectorSize::fourKilobytes)
...
template <MediaSectorSize N> auto Media<N>::setFileSystemTime() -> Error
{
...
Media<N>::Media(const DriverCallback driverCallback,
                std::byte *driverInfoPtr,
                const NotifyCallback openNotifyCallback,
                const NotifyCallback closeNotifyCallback)
    : ThreadX::Native::FX_MEDIA{},
      m_driverCallback{std::move(driverCallback)},
      m_driverInfoPtr{driverInfoPtr},
      m_openNotifyCallback{std::move(openNotifyCallback)},
      m_closeNotifyCallback{std::move(closeNotifyCallback)}
{
...
template <MediaSectorSize N> auto Media<N>::open(const std::string_view name, const FaultTolerantMode mode) -> Error
{
    using namespace ThreadX::Native;

    if (Error error{fx_media_open(this,
...
template <MediaSectorSize N>
auto Media<N>::format(const std::string_view volumeName,
                      const ThreadX::Ulong storageSize,
                      const ThreadX::Uint sectorPerCluster,
...
template <MediaSectorSize N> auto Media<N>::volume(const std::string_view volumeName) -> Error
{
    assert(volumeName.length() < volumNameLength);
    return Error{fx_media_volume_set(this, const_cast<char *>(volumeName.data()))};
}

template <MediaSectorSize N> auto Media<N>::volume() -> ExpectedStr
{
...
template <MediaSectorSize N> auto Media<N>::createDir(const std::string_view dirName) -> Error
{
    return Error{fx_directory_create(this, const_cast<char *>(dirName.data()))};
}

template <MediaSectorSize N> auto Media<N>::deleteDir(const std::string_view dirName) -> Error
{
...
template <MediaSectorSize N> auto Media<N>::createFile(const std::string_view fileName) -> Error
{
    return Error{fx_file_create(this, const_cast<char *>(fileName.data()))};
}

template <MediaSectorSize N> auto Media<N>::deleteFile(const std::string_view fileName) -> Error
{
...
template <MediaSectorSize N> auto Media<N>::defaultDir(const std::string_view newPath) -> Error
{
    return Error{fx_directory_default_set(this, const_cast<char *>(newPath.data()))};
}

template <MediaSectorSize N> auto Media<N>::defaultDir() -> ExpectedStr
{
    char *path = nullptr;
...
template <MediaSectorSize N> auto Media<N>::localDir(const std::string_view newPath) -> Error
{
    using namespace ThreadX::Native;
...
template <MediaSectorSize N> auto Media<N>::localDir() -> ExpectedStr
{
    char *path = nullptr;
...
template <MediaSectorSize N> auto Media<N>::clearLocalDir() -> Error
{
    return Error{fx_directory_local_path_clear(this)};
}

template <MediaSectorSize N> auto Media<N>::space() -> ExpectedUlong64
{
...
template <MediaSectorSize N> auto Media<N>::abort() -> Error
{
    return Error{fx_media_abort(this)};
}

template <MediaSectorSize N> auto Media<N>::invalidateCache() -> Error
{
    return Error{fx_media_cache_invalidate(this)};
}

template <MediaSectorSize N> auto Media<N>::flush() -> Error
{
    return Error{fx_media_flush(this)};
}

template <MediaSectorSize N> auto Media<N>::close() -> Error
{
    return Error{fx_media_close(this)};
}

template <MediaSectorSize N> auto Media<N>::name() const -> std::string_view
{
...
template <MediaSectorSize N>
Media<N>::InternalDriver::InternalDriver(Media &media)
    : m_media{media}
{
}

template <MediaSectorSize N> consteval auto Media<N>::InternalDriver::sectorSize() -> MediaSectorSize
{
    return N;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::info() const -> void *
{
    // Access the member via the containing FX_MEDIA pointer
    return m_media.fx_media_driver_info;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::request() const -> MediaDriverRequest
{
    return MediaDriverRequest{m_media.fx_media_driver_request};
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::status(const Error error) -> void
{
    m_media.fx_media_driver_status = std::to_underlying(error);
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::buffer() const -> ThreadX::Uchar *
{
    return m_media.fx_media_driver_buffer;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::logicalSector() const -> ThreadX::Ulong
{
    return m_media.fx_media_driver_logical_sector;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::sectors() const -> ThreadX::Ulong
{
    return m_media.fx_media_driver_sectors;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::writeProtect(const bool writeProtect) -> void
{
    m_media.fx_media_driver_write_protect = writeProtect;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::freeSectorUpdate(const bool freeSectorUpdate) -> void
{
    m_media.fx_media_driver_free_sector_update = freeSectorUpdate;
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::systemWrite() const -> bool
{
    return static_cast<bool>(m_media.fx_media_driver_system_write);
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::dataSectorRead() const -> bool
{
    return static_cast<bool>(m_media.fx_media_driver_data_sector_read);
}

template <MediaSectorSize N> auto Media<N>::InternalDriver::sectorType() const -> MediaSectorType
{
    return MediaSectorType{m_media.fx_media_driver_sector_type};
}

template <MediaSectorSize N> auto Media<N>::driverCallback(auto mediaPtr) -> void
{
    auto &media{static_cast<Media &>(*mediaPtr)};
    media.m_driverCallback(media.m_internalDriver);
}

template <MediaSectorSize N> auto Media<N>::openNotifyCallback(auto mediaPtr) -> void
{
    auto &media{static_cast<Media &>(*mediaPtr)};
    media.m_openNotifyCallback(media);
}

template <MediaSectorSize N> auto Media<N>::closeNotifyCallback(auto mediaPtr) -> void
{
    auto &media{static_cast<Media &>(*mediaPtr)};
